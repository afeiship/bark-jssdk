...
describe('api.basic', () => {
  ...
  test('api: message should get promise', async () => {
    ...
    const result = await sdk.msg({ title: '提示', body: '您有新消息来了' });
  ...
  test('api: with sound', async () => {
    const sdk = new BarkJssdk();
    const result = await sdk.msg({ title: '推送铃声', body: '我会带着sound', sound: IOS_SOUND.mailsent });
    expect(typeof sdk.msg).toBe('function');
    expect(result.message).toBe('success');
  });
});
